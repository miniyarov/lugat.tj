...
<div class="innerHTML">
<?php
	$q = "SELECT * FROM last_searched_word ORDER BY time DESC LIMIT 20";
	$r = mysql_query($q,$connection);
	if (!$r) {
		die("SQL Error: ".mysql_error());
	}
	while($row = mysql_fetch_array($r)){
		$country = geoip($row['ip_address']);
		$t = _ago($row['time'],$ln);
		echo "<a href='index.php?ln={$ln}&word=".htmlspecialchars($row['word'])."'>".htmlspecialchars($row['word'])."</a><div class='right gray'>{$country[$ln]}, {$t} nazad</div><br>";
	}
	mysql_free_result($r);
?>
...
<div class="innerHTML">
<?php
	$q = "SELECT word,counter,ip_address FROM last_searched_word ORDER BY counter DESC LIMIT 20";
	$r = mysql_query($q,$connection);
	if (!$r) {
		die("SQL Error: ".mysql_error());
	}
	while($row = mysql_fetch_array($r)){
		$country = geoip($row['ip_address']);
		echo "<a href='index.php?ln={$ln}&word=".htmlspecialchars($row['word'])."'>".htmlspecialchars($row['word'])."</a><div class='right'>{$country[$ln]}</div><br>";
	}
	mysql_free_result($r);
?>
</div>
