...
function _ago($tm,$ln = 'ru',$rcs = 0) {
	$tm = strtotime($tm);
    $cur_tm = time(); 
	$dif = $cur_tm-$tm;
	switch ($ln) {
		case 'ru':$pds = array('секунда','минута','час','день','неделя','месяц','год','десятилетие');
		case 'en':$pds = array('second','minute','hour','day','week','month','year','decade');
		case 'tj':$pds = array('секунда','минута','час','день','неделя','месяц','год','десятилетие');
		default:$pds = array('секунда','минута','час','день','неделя','месяц','год','десятилетие');
	}
